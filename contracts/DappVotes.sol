...
contract DappVotes {
    ...
    function getPolls() public view returns (PollStruct[] memory) {
        uint available;
        for (uint i = 0; i < totalPolls.current(); i++) {
            if (!polls[i].deleted) available++;
        }

        PollStruct[] memory pollArray = new PollStruct[](available);
        uint index;

        for (uint i = 0; i < totalPolls.current(); i++) {
            if (!polls[i].deleted) {
                pollArray[index++] = polls[i];
            }
        }
        return pollArray;
    }
    ...
    function getContestants(uint id) public view returns (ContestantStruct[] memory) {
        uint available;
        for (uint i = 0; i < totalContestants.current(); i++) {
            if (contestants[id][i].id == i) available++;
        }

        ContestantStruct[] memory contestantArray = new ContestantStruct[](available);
        uint index;

        for (uint i = 0; i < totalContestants.current(); i++) {
            if (contestants[id][i].id == i) {
                contestantArray[index++] = contestants[id][i];
            }
        }
